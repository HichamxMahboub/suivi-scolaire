    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg animate-fade-in">
            <div class="p-6 text-gray-900">
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h1 class="text-3xl font-bold text-brand-700 mb-2 font-sans flex items-center gap-2">
                            <span class="inline-block align-middle">🧑‍💼</span> Modifier l'encadrant
                        </h1>
                        <p class="text-gray-600 font-sans">Modifiez les informations de {{ $encadrant->nom }} {{ $encadrant->prenom }}</p>
                    </div>
                    <div class="flex space-x-2">
                        <a href="{{ route('encadrants.show', $encadrant) }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-brand-400" aria-label="Voir l'encadrant">
                            Voir l'encadrant
                        </a>
                ...
                <!-- Formulaire -->
                <form method="POST" action="{{ route('encadrants.update', $encadrant) }}" class="space-y-8 font-sans" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="bg-gray-50 p-6 rounded-2xl shadow-md border-l-4 border-brand-400 flex flex-col gap-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="mb-4">
                                ...
                                <input type="text" name="nom" id="nom" value="{{ old('nom', $encadrant->nom) }}" required
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="prenom" id="prenom" value="{{ old('prenom', $encadrant->prenom) }}" required
                                    class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand-600 focus:border-transparent transition-all">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="matricule" id="matricule" value="{{ old('matricule', $encadrant->matricule) }}" required
                                    class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand-600 focus:border-transparent transition-all">
                                @error('matricule')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div class="mb-4">
                                <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Email *</label>
                                <input type="email" name="email" id="email" value="{{ old('email', $encadrant->email) }}" required
                                    class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand-600 focus:border-transparent transition-all">
                                @error('email')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div class="mb-4">
                                <label for="telephone" class="block text-sm font-medium text-gray-700 mb-1">Téléphone</label>
                                <input type="text" name="telephone" id="telephone" value="{{ old('telephone', $encadrant->telephone) }}"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand-600 focus:border-transparent transition-all">
                                @error('telephone')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div class="mb-4">
                                <label for="adresse" class="block text-sm font-medium text-gray-700 mb-1">Adresse</label>
                                <input type="text" name="adresse" id="adresse" value="{{ old('adresse', $encadrant->adresse) }}"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand-600 focus:border-transparent transition-all">
                            ...
                            <div class="mb-4 md:col-span-2">
                                ...
                                <textarea name="remarque" id="remarque" rows="2"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-brand-600 focus:border-transparent transition-all"
                                    placeholder="Remarque sur l'encadrant">{{ old('remarque', $encadrant->remarque) }}</textarea>
