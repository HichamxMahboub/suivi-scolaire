...
<aside id="sidebar" class="fixed top-0 right-0 h-full w-[220px] z-40 flex flex-col transition-transform duration-300 rounded-l-2xl closed">
    <div id="sidebar-content" style="margin-top: 5rem;">
        ...
        <nav class="flex-1 px-0 py-2 space-y-1">
            ...
            <a href="{{ route('notes.index') }}" class="flex items-center gap-3 px-6 py-3 text-gray-900 font-medium text-base transition rounded-xl @if(request()->routeIs('notes.*')) sidebar-active @endif">
                <span class="w-8 text-center text-xl"><i class="fas fa-clipboard-list"></i></span> <span>Notes</span>
            </a>
            ...
            <a href="{{ route('users.index') }}" class="flex items-center gap-3 px-6 py-3 text-gray-900 font-medium text-base transition rounded-xl @if(request()->routeIs('users.*')) sidebar-active @endif">
                <span class="w-8 text-center text-xl"><i class="fas fa-user-cog"></i></span> <span>Utilisateurs</span>
            </a>
            @endif
        ...
        <div class="mt-auto flex flex-col items-center py-6">
            ...
            <div class="text-gray-900 text-center">
                <div class="font-semibold text-base">{{ Auth::user()?->name ?? 'Utilisateur' }}</div>
                <div class="text-xs opacity-80">{{ Auth::user()?->email ?? '' }}</div>
                @if(Auth::user())
                <div class="text-xs opacity-70 mt-1">
                    @if(Auth::user()->isAdmin()) 👨‍💼 Administrateur
                    @elseif(Auth::user()->isEncadrant()) 🎓 Encadrant
                    @elseif(Auth::user()->isMedical()) 🏥 Personnel médical
                    @elseif(Auth::user()->role === 'teacher') 📚 Enseignant
                    @elseif(Auth::user()->role === 'student') 🎒 Élève
                    @else 👤 Utilisateur
                    @endif
                </div>
                @endif
